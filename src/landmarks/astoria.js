...
/** Astoria Park, the East River, Manhattan's skyline, the bridges, the stacks, the ASTORIA sign. */
export function buildAstoriaLandmarks(layout, shared) {
  const { NX, colX, rowZ } = D;
  const PARK_Z0 = D.parkZ0;
  const PARK_Z1 = D.parkZ1;
  const RIVER_X = D.riverX;
  const EAST_LIMIT = D.xMax;
